...
| SR-005 | High | The n8n edition is now known, but safe Middleware endpoint/egress, service-owned credential binding, and complete editor-session controls are not verified. | `config/n8n-policy.json` remains `UNVERIFIED`; see `docs/evidence/N4/RUNTIME-POLICY-AUDIT.md`. Templates stay disabled at `middleware.invalid` with no credential. |
| SR-006 | Blocker | User-provided Code can expose credentials and instance data without isolated task runners. Live production/staging evidence shows `NODES_EXCLUDE` absent, and staging has internal JavaScript runners active. | Keep executable exports blocked. Apply a separately authorized runtime change that excludes dangerous nodes in every role, or deploy and certify hardened external runners before enabling Code. |
